...
\title{
	\bfseries PROJECT 2: 
	Diagnosing permafrost state in Pangnirtung (Nunavut, Canada)\\
	
	An analysis from satellite and ground-based thermic measurements\\
	
	\large
	Advanced Remote Sensing - Natural Hazards and Climate Change\\
	GEO4520 - University of Oslo
}
\author{
...
A possible pseudocode for such an algorithm is Algorithm \ref{alg:ff}
\begin{algorithm}
	\color{BlueViolet}
...
\paragraph{Implementation}
Note that, in order to run backpropagation, the feedforward algorithm A possible pseudocode for the backpropagation algorithm is shown in the Algorithm \ref{alg:bp}.
\begin{algorithm}
	\color{BlueViolet}
...
\begin{figure*}
	\centering
	\includegraphics[width=0.943\textwidth]{schedulers_mse_r2}
...
\[
\begin{split}
	\text{BCE}(\bs{y}, \bs{\tilde{y}}, \bs{\Theta})=\\ = -\frac{1}{N} \sum_{i=1}^N \left[ y_i \, \log(p_i) + (1 - y_i) \,\log(1 - p_i) \right]
\end{split}
\]
with derivative:
\begin{equation}
...
\textbf{1. Loading data, preprocessing}\\
	...
	Both the datasets have been scaled; for the Runge dataset we employed \texttt{StandardScaler} from \texttt{scikit-learn.utils}. For the MNIST data, since we were dealing with pixel values in a gray-scale range [0, 255], we normalized all the pixels to the interval [0, 1], dividing by 255.
