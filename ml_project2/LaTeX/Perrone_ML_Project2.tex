...
For this project, a Python module has been created, containing all the necessary functions and classes for the analysis. The core is a Neural Network class, that can perform all the main algorithms (feedforward, backpropagation, training) and adapt to different types of input dataset and regression/classification frames.

Since we deal with learning machinery, Python libraries such as \texttt{scikit-learn} and \texttt{pytorch} have also been employed, mainly for assessing the relative behavior of the own coded NN with respect to external modules.
...
\begin{figure*}
	\centering
	\includegraphics[width=14cm]{ols_Vs_nn}
...
In Figure \ref{fig:nnols}, the result of a regression performed with both the \py{NeuralNetwork} and the \py{LinearRegression\_own} classes. The second one has been set with a polynomial degree of 13; this choice was made in accord with the Bias-Variance tradeoff results from the analyses carried out in Project 1. 
...
First of all, the speed of each algorithm has been tested, by repeating for 50 iterations the same training with a NN of 2 hidden layers with 100 nodes each, all activated by a sigmoid. For all the algorithms, an initial learning rate of $\num{5e-3}$ and 500 epochs have been set. For each iteration, the execution time was registered. An average elapsed time over the 50 iterations was finally calculated for each scheduler, and the result can be seen in Table \ref{tab:schedtimes}. RMSProp resulted in average 1.4 times slower than the plain SGD, and ADAM 1.7 times slower.
\begin{table}
	\centering
	\begin{tabular}{lSS}
		\toprule
		...
		\bottomrule
	\end{tabular}
	\caption{\textbf{Different execution time for each SGD algorithm} | ADAM and RMSProp are slower than the plain SGD. A standard deviation of the mean is also shown.}
	\label{tab:schedtimes}
\end{table}
\begin{figure*}
	\centering
	\includegraphics[width=\textwidth]{schedulers_mse_r2}
	\caption{\textbf{Performance of different SGD solvers} | RMSProp and ADAM converge to acceptable results within a smaller amount of epochs, proving a fairly consistent behavior to any learning rate and number of iterations. On the opposite, plain SGD with constant learning rate seems performing better with lower rates and a higher number of epochs.}
	\label{fig:sgdnn}
\end{figure*}

Secondly, the overall effectiveness of the three schedulers has been investigated as function of learning rate and number of epochs. The result of the analysis can be observed in Figure \ref{fig:sgdnn}. Despite taking a longer time to be executed, RMSProp and ADAM show more than acceptable MSE and $R^2$ even with a relatively low number of epochs, \emph{i.e.} they converge faster to an optimal value for the parameters.

\subsection{Custom NN and external libraries}
We compared the the speed ad effectiveness of our NN with the benchmark \texttt{MPLregressor} provided by \sk. With the same characteristics, \emph{i.e.} two hidden layers activated by a tanh, initial learning rate of \num{1e-3} and small L2 regularizations, the difference in the execution time is huge. For both, ADAM has been used as optimizer. Our NN completed the task in $\sim \qty{11.5}{\second}$, whereas \sk performed the same task in $\sim \qty{0.4}{\second}$, \ie it is 29 times faster. The resulting fits are similar, even though our NN resulted in a slightly better approximation on the test set ($R_\text{own}^2 = 0.9995$, $R_\text{sk-learn}^2 = 0.9932$). The two fits are shown in Figure \ref{fig:skvsown}.

Moreover, we made sure to obtain the same gradients with \texttt{autograd}, setting the \texttt{autodiff} parameter to True, and with the manually computed ones. Two NNs with the same set up but with different different differentiation methods yielded exactly the same fits.
\begin{figure*}
	\centering
	\includegraphics[width=12cm]{skvsown}
	\caption{\textbf{Custom NN vs \sk} | The two algorithms yield a similar result, even though our NN is 29 times slower.}
	\label{fig:skvsown}
\end{figure*}

\subsection{Adding L1 and L2 regularizations}
The results of the two regularized fits are shown in Figure \ref{fig:nnl1l2}. We can clearly see that acceptable result are obtained, in both cases, for $\lambda\leq \num{e-3}$ and for $\eta\leq \num{5e-5}$. We can observe that there is no gradual decay in the quality of MSE and $R^2$: the transitions is very sharp at these two critical thresholds.
\begin{figure*}
	\centering
...
MNIST-784 have been correctly loaded, train-test split and scaled. Four samples from this collection can be observed in Figure \ref{fig:mnistsamples}.
\begin{figure}
	\centering
	\includegraphics[width=\columnwidth]{mnist_samples}
	\caption{\textbf{Samples from \href{https://www.kaggle.com/datasets/hojjatk/mnist-dataset}{MNIST-784} collection} | The hand-written digits are 28$\times$28 pixel sized and each pixel has gray scale value ranging from 0 to 255.}
\end{figure}
The classification of these data has been performed with NN of two hidden layers of size 128 and 64 nodes respectively. The Leaky ReLU has been chosen as activation function of these layers, whereas for the output a Softmax has been employed. This set up is known to yield the best results when it comes to multi-class classifications. ADAM has been set as optimizer, with 1000 epochs and learning rate of \num{e-3}, since this is one of the best combination according to previous results (Section \ref{ss:sched}).

The classification was successful: the result is shown in the confusion matrix of Figure \ref{fig:cm}. We can see that a high percentage of samples have been correctly classified, as they fall on the diagonal of the matrix.
\begin{figure}
	\centering
	\includegraphics[width=\columnwidth]{mnist_cm}
	\caption{\textbf{Confusion matrix of MNIST data classification} | Good result, with overall accuracy of 0.92.}
	\label{fig:cm}
\end{figure}
The accuracy of the classification is of 0.92, which is more than acceptable.

\section{Discussion}
The custom NN performed overall as predicted. It has been able to correctly approximate simple functions and classify hand-written digits.

When it comes to simple regressions, the choice of a linear regressor such as the Ordinary Least Squares (OLS) is still a solid choice, for its relatively simple implementation and fast execution. The equations that define the model are usually straightforward and are characterized by a limited number of parameters \parencite{wooditch2021_ols}. However, the main limitation of such methods is that they assume a linear relationships between the dependent and the independent variable. For functions such as the Runge one, this can represent a problem, since a polynomial approximation will not be able to properly catch the non-linearities of the relationship. NNs, thanks to the non-linear activation functions that can include, are potentially a better choice for this type of systems \parencite{deridder1999_nn, waterworth2000_nn}. This can directly be observed in Figure \ref{fig:nnols}: even a complex polynomial OLS model cannot perform well as a relatively simple, two-hidden layers NN, that exactly identify non linearities in the given function. Moreover, modern training techniques such as stochastic gradient descent and regularization strategies mitigate overfitting while maintaining model effectiveness \cite{lecun1998_tanh}.

Choosing a NN over a linear regressor can still have drawbacks: it can take some time to find the best tradeoff between complexity and efficiency, and very deep networks will take a relevant amount of time to be executed. In our case, the OLS regression fitted the Runge dataset in \qty{0.0011}{\second}, whereas the two-hidden layer NN with 100 nodes and 1000 epochs required \qty{4.58}{\second}, that is around 4000 times slower.

Nevertheless, our results showed that highly efficient algorithm from external libraries are able to tackle the same tasks within a much smaller time interval, and this raises one of the main criticalities of our custom code: despite the overall consistent results, the code is still far from being considered fine-tuned. 

In the because it assumes a continuous output space and does not constrain predictions to lie within class probability bounds (e.g., between 0 and 1). As a result, linear regression can yield invalid probability estimates and poorly defined decision boundaries \parencite{hosmer2013applied, bishop2006_pattern}. Logistic regression and other probabilistic models are preferred alternatives, as they explicitly model the conditional class probabilities through non-linear link functions.
