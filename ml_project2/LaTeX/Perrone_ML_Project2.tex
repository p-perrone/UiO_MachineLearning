...
\begin{figure*}
	...
	\caption{\textbf{Simple perceptron mode} | In blue the different layers, in orange the functions.}
...
A consistent result from the NN calculations can only be achieved with properly trained parameters $\bs{W}$ and $\bs{b}$. As in every machine learning, the optimization of such parameters can be performed by minimizing the gradient of the cost function with respect to these same. \emph{Backpropagation} is the operation of calculating these gradients. For the layer $l$:
\begin{equation}
...
\begin{equation}
	\bs{\delta}^{(L)} = f'(\bs{Z}^{(L)}) \circ \pdv{\C}{\bs{A}^{(L)}}
\end{equation}
where $\circ$ represents the element-wise (Hadamard) product.
Finally, we can write the derivative of the cost function with respect to the weights of the last layer is equal to:
\begin{equation}
...
In order to obtain the expression of backpropagation at a generic layer 
